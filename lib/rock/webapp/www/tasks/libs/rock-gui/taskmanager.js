...
//		});
	...
	this.addPort = function(taskname, portinfo){
		...
		if (portinfo.direction == "output"){
			...
			getTypeInfoOf(porturl,function(portinfo){
				...
				var id = taskmgr.id + taskname.replace(/\//g,"") + portinfo.name + "data";
				var portdata = document.getElementById(id);

				//generate a html from from the type information 
				
				var form = generateForm(porturl+"/write", portinfo, id);

				portdata.appendChild(form);
				
				createInputElement(portinfo.type.name, porturl, 640, 640, form);
	...
	this.setPortData = function(taskname, portinfo){
		var portentry = document.getElementById(this.id+taskname+"/"+portinfo.name);
			...
			var value;
			var dataid = this.id+taskname.replace(/\//g,"") + portinfo.name;
	...
	this.updatePortValue = function(taskname, portname){
		...
		getPort( this.url+"/tasks/"+taskname+"/ports/"+portname ,function(portinfo){
			...
			var url = taskmgr.url+"/tasks/"+taskname+"/ports/"+portinfo.name;
			...
			readPort(readurl, function(data){
				...
				//console.log(data) 
				var id = taskmgr.id+ taskname.replace(/\//g,"") + portinfo.name;
				...
				var websock_url = readurl.replace(/http:\/\//,"ws://");
				var play = createButton("play","window['"+taskmgr.id+"'].constantUpdate(\""+taskname+"\",\""+portname+"\")");
				var stop = createButton("stop","window['"+taskmgr.id+"'].stopConstantUpdate(\""+taskname+"\",\""+portname+"\")");
				...
				if (text[0] == '{'){
					...
					$('#'+id+"data").JSONView(text, {collapsed: false});			
				}else{
					portentry.setAttribute("style","color:blue");
					while (portentry.hasChildNodes()) {
						portentry.removeChild(portentry.lastChild);
					}
					portentry.appendChild(document.createTextNode(text));	
				}
				
				createOutputElement(portinfo.type.name, url, 640, 640, portentry);
				
			});
		});
	};
	
	this.stopConstantUpdate = function(taskname, portname){
		var readurl = this.url+"/tasks/"+taskname+"/ports/"+portname+"/read";
		var websock_url = readurl.replace(/http:\/\//,"ws://");
		deleteWebSocket(websock_url);
	}
	
	this.constantUpdate = function(taskname, portname){
		var taskmgr = this;
		
		getPort( taskmgr.url+"/tasks/"+taskname+"/ports/"+portname ,function(portinfo){
			
			var readurl = taskmgr.url+"/tasks/"+taskname+"/ports/"+portinfo.name+"/read";
			var websock_url = readurl.replace(/http:\/\//,"ws://");
			var targetid = taskmgr.id+ taskname.replace(/\//g,"") + portinfo.name + "data";
			
			createWebSocket(websock_url, function(content){
				var text = getPortContentAsText(portinfo, content, "");
				taskmgr.updatePortContent(targetid,text);
			});			
			
		});	
	}
	
	this.updatePortContent = function(targetid, content){
		var portentry = document.getElementById(targetid);
		if (content[0] == '{'){
			//console.log("got data");
			$('#'+targetid).JSONView(content, {collapsed: false});			
		}else{
			portentry.innerHTML = content;	
		}
	}
